...
const ProductDetail = () => {
    ...
    const { data: product, isLoading: productIsLoading, isError: productIsError } = useProductQuery({ tag });
    const { data: categoryData, isLoading: categoryIsLoading, isError: categoryIsError } = useCategoryQuery();
    ...
    const handleUpdateProduct = async (event: any) => {
        ...
        try {
            const {error,value} = productValidate(updatedProduct);
            if (error) {
                alert(error);
            } else {
                await mutate(value);
                                ...
                                <div className="p-7">
                                    <div className="mb-5 flex flex-col gap-5 sm:flex-row">
                                        <div className="w-full sm:w-1/2">
                                            <label className="mb-3 block text-sm font-medium text-black " htmlFor="fullName">
                                                Name Product</label>
                                            <input onChange={handleChange} defaultValue={product.name} className="w-full rounded border border-stroke bg-[#F1F5F9] py-3 p-4 font-normal text-black focus:border-primary focus-visible:outline-none dark:border-strokedark dark:bg-meta-4  dark:focus:border-primary" type="text" name="name" id="name" placeholder="product" />
                                        </div>
                                        <div className="w-full sm:w-1/2 md:ml-6">
                                            ...
                                            <input onChange={handleChange} defaultValue={product.inStock} className="w-full rounded border border-stroke bg-[#F1F5F9] py-3 px-4 font-normal text-black focus:border-primary focus-visible:outline-none dark:border-strokedark dark:bg-meta-4  dark:focus:border-primary" type="number" min={1} name="inStock" id="inStock" placeholder="stock" />
                                        </div>
                                    </div>
                                    <div className="my-5">
                                    ...
                                    <div className="mb-5 flex flex-col gap-5 sm:flex-row">
                                        <div className="sm:w-1/2 w-full mr-4">
                                            ...
                                            <input onChange={handleChange} defaultValue={product.price} className="w-full rounded border border-stroke bg-[#F1F5F9] py-3 px-4 font-normal text-black focus:border-primary focus-visible:outline-none dark:border-strokedark dark:bg-meta-4  dark:focus:border-primary" type="number" min={0} name="price" id="price" placeholder="price" />
                                        </div>
                                        <div className="sm:w-1/2 w-full">
                                            <label className="mb-3 block text-sm font-medium text-black " htmlFor="emailAddress">Discount</label>
                                            <input onChange={handleChange} defaultValue={product.discount} className="w-full rounded border border-stroke bg-[#F1F5F9] py-3 p-4 font-normal text-black focus:border-primary focus-visible:outline-none dark:border-strokedark dark:bg-meta-4  dark:focus:border-primary" type="number" min={0} name="discount" id="discount" placeholder="discount" />
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div className="p-7">
                                    <div id="FileUpload" className="relative mb-5 block w-full cursor-pointer appearance-none rounded border-2 border-dashed border-primary bg-[#F1F5F9] py-4 px-4 dark:bg-meta-4 sm:py-7.5">
                                        ...
                                        <div className="flex flex-col items-center justify-center space-y-3">
                                            ...
                                            <img className="max-w-72 max-h-72" src={`/assets/image/product/${file ? file : product.image}`} alt="" />
                                        </div>
                                    </div>
                                    <div className="flex justify-center gap-5">
                                        <button onClick={handleUpdateProduct} className="flex justify-center rounded bg-sky-600 py-2 px-6 font-normal text-gray hover:bg-opacity-90" type="submit">
                                            Update Product
                                        </button>
                                    </div>
                                </div>
