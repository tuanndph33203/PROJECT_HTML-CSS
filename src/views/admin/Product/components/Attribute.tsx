...
const ColorPicker = ({attributes,setAttributes}:any) => {
    ...
    const addColor = () => {
        if (selectedColor && !attributes.find((item: ColorItem) => item.nameValue === selectedColor)) {
            if (quantity > 0) {
                setAttributes([...attributes, { nameValue: selectedColor, stock: quantity }]);
        ...
        <div className="order-2 col-span-5 xl:col-span-2">
            <div className="rounded-sm bg-white border border-stroke bg-shadow-default dark:border-strokedark">
                ...
                <div className="border-b border-stroke py-4 px-7 dark:border-strokedark">
                    ...
                    <div className="mb-5 flex flex-col gap-5 sm:flex-row items-end">
                        <div className="w-full sm:w-7/12">
                            <label className="mb-3 block text-sm font-medium text-black" htmlFor="quantity">Quantity</label>
                            <input
                    ...
                    <ul>
                        {attributes.map((colorItem : any ,index : number)=> (
                            <li key={index} className="grid grid-cols-3 mb-2 text-center">
                                <div className="flex items-center">
                                    <div className="w-8 h-8 mr-4 cursor-pointer" style={{ backgroundColor: colorItem.nameValue }}></div>
                                    <span>{colorItem.nameValue}</span>
                                </div>
                                <div className="flex items-center">
                                    <span className='mr-4 font-medium'>Quantity</span>
                                    <input
                                        readOnly
                                        className="w-[100px] rounded border border-stroke bg-[#F1F5F9] py-1 px-2 font-normal text-black focus:border-primary focus-visible:outline-none dark:border-strokedark dark:bg-meta-4 dark:focus:border-primary"
                                        type="number"
                                        min={1}
                                        name="quantity"
                                        id="quantity"
                                ...
                                <div className="remove flex items-center justify-end">
